...
- **Insight snapshots** — `--suite insights --spec-engine lune` runs the
  telemetry insight spec, captures the latest tuning health snapshot, and writes
  `tests/artifacts/insights/telemetry-insights.json` for quick comparison
  between branches.
...
- `insights/telemetry-insights.json` — summarises success rates, confidence
  recommendations, and pending config deltas generated by
  `AutoParry.getTelemetryInsights()`.
